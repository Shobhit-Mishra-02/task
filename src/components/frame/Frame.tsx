    ...
    >
      <div
        className={`card__img_heading ${
          isSelected ? "card__img_heading_translate" : ""
        }`}
      >
...
function AvatarFrame({
  ...
  useEffect(() => {
    if (location.length && imageUrl.length) {
      if (isDisabled) {
        setDisabled(false);
      }
    } else {
  ...
  return (
    <form onSubmit={handleSubmission} className="avatar_frame__container">
...
function Frame() {
  ...
  const renderPage = () => {
    if (page === 1) {
      return <AvatarFrame setPage={setPage} />;
    } else if (page === 2) {
      return <DribbbleFrame />;
    } else {
  ...
  return (
    <div className="frame">
      <div className="frame__navbar">
        ...
        {page === 2 ? (
          <div onClick={() => setPage(1)} className="prev_icon__container">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
